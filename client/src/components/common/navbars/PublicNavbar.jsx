...
// Renders the public-facing navigation bar for unauthenticated visitors, providing access to general site sections and login
export function PublicNavbar({ forceDark = false }) {
    ...
    const borderColorClass = shouldUseDark ? "border-gray-200" : "border-white/30";
            ...
            >
                <div className="container mx-auto px-6 md:px-12 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                                to={link.path}
                                className={`relative text-sm font-medium tracking-wide transition-colors group-hover:text-gray-300 hover:!text-white ${shouldUseDark ? "text-gray-600" : "text-white/90"}`}
                            >
                                {link.name}
                    ...
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        <Link to="/login">
                            <span className={`text-sm font-semibold tracking-wide border-b pb-0.5 hover:opacity-70 transition-all duration-300 cursor-pointer group-hover:text-white group-hover:border-white ${textColorClass} ${shouldUseDark ? "border-neutral-900" : "border-white"}`}>
                                LOG IN
                            </span>
                        </Link>
                    </div>
